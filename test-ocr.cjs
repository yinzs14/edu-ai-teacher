...
// 读取 .env（处理 UTF-16 LE 编码）
const rawBuffer = fs.readFileSync('.env');
let envContent;

// 检测 BOM
if (rawBuffer[0] === 0xFF && rawBuffer[1] === 0xFE) {
  // UTF-16 LE with BOM
  envContent = rawBuffer.toString('utf16le').replace(/^\uFEFF/, '');
} else if (rawBuffer[0] === 0xFE && rawBuffer[1] === 0xFF) {
  // UTF-16 BE with BOM
  envContent = rawBuffer.toString('utf16be').replace(/^\uFEFF/, '');
} else {
  // UTF-8 or other
  envContent = rawBuffer.toString('utf8').replace(/^\uFEFF/, '');
}
...
async function getAccessToken() {
  ...
  if (!apiKey || !secretKey) {
    console.log('BAIDU_API_KEY:', apiKey || '(missing)');
    console.log('BAIDU_SECRET_KEY:', secretKey ? '***' : '(missing)');
...
async function main() {
  ...
  if (!imagePath) {
    console.log('Usage: node test-ocr.cjs <image-path>');
    console.log('Example: node test-ocr.cjs "C:\Users\63435\.openclaw\workspace\downloads\19e72dfd-3262-8934-8000-0000c609d696_image.png"');
    return;
  }
